...
$query =mysqli_query($conection,"INSERT INTO `Motores` (`Foto`, `Nombre`, `Descripcion`, `Precio`)
   ...
   header('location: MantMotores.php');
    break;
     case "btnModificar":
 ...
 $query =mysqli_query($conection," UPDATE `Motores` SET `Foto` = '$nombreArchivo', `Nombre` = '$txtNombre', `Descripcion` = '$txtDescripcion', `Precio` = '$txtPrecio' WHERE `Motores`.`ID_Motor` = '$txtID'");
 echo '<script language="javascript">alert("Motores Modificado Correctamente");</script>';
header('location: MantMotores.php');

    break;
     case "btnEliminar":

$query =mysqli_query($conection,"DELETE FROM `Motores` WHERE `Motores`.`ID_Motor` = $txtID");
   echo '<script language="javascript">alert("Agregada Nueva Motores");</script>';
  header('location: MantMotores.php');
...
<head>
  <meta charset="utf-8">
  <title>Mantenimiento Motores</title>
...
<div class="container">
  <br>
  <h1>Mantenimiento para Motores</h1>
  <br>
  <form action="" method="post" enctype="multipart/form-data">

   <label for="name">ID Motores</label>
