...
$query =mysqli_query($conection,"INSERT INTO `Tuning` (`Foto`, `Nombre`, `Descripcion`, `Precio`)
   ...
   header('location: MantTuning.php');
    break;
     case "btnModificar":
 ...
 $query =mysqli_query($conection," UPDATE `Tuning` SET `Foto` = '$nombreArchivo', `Nombre` = '$txtNombre', `Descripcion` = '$txtDescripcion', `Precio` = '$txtPrecio' WHERE `Tuning`.`ID_Tuning` = '$txtID'");
 echo '<script language="javascript">alert("Tuning Modificado Correctamente");</script>';
header('location: MantTuning.php');

    break;
     case "btnEliminar":

$query =mysqli_query($conection,"DELETE FROM `Tuning` WHERE `Tuning`.`ID_Tuning` = $txtID");
   echo '<script language="javascript">alert("Agregada Nueva Tuning");</script>';
  header('location: MantTuning.php');
...
<head>
  <meta charset="utf-8">
  <title>Mantenimiento Tuning</title>
...
<div class="container">
  <br>
  <h1>Mantenimiento para Tuning</h1>
  <br>
  <form action="" method="post" enctype="multipart/form-data">

   <label for="name">ID Tuning</label>
