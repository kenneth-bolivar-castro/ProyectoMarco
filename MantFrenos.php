...
$query =mysqli_query($conection,"INSERT INTO `Frenos` (`Foto`, `Nombre`, `Descripcion`, `Precio`)
                                       VALUES('$nombreArchivo','$txtNombre', '$txtDescripcion', '$txtPrecio')");

   header('location: MantFrenos.php');
    break;
 ...
 $query =mysqli_query($conection," UPDATE `Frenos` SET `Foto` = '$nombreArchivo', `Nombre` = '$txtNombre', `Descripcion` = '$txtDescripcion', `Precio` = '$txtPrecio' WHERE `Frenos`.`ID_Frenos` = '$txtID'");
...
$query =mysqli_query($conection,"DELETE FROM `Frenos` WHERE `Frenos`.`ID_Frenos` = $txtID");
...
<head>
  <meta charset="utf-8">
  <title>Mantenimiento Frenos</title>
...
<div class="container">
  <br>
  <h1>Mantenimiento para Frenos</h1>
  <br>
  <form action="" method="post" enctype="multipart/form-data">

   <label for="name">ID Frenos</label>
