...
$query =mysqli_query($conection,"INSERT INTO `Electrico` (`Foto`, `Nombre`, `Descripcion`, `Precio`)
   ...
   header('location: MantElectrico.php');
    break;
     case "btnModificar":
 ...
 $query =mysqli_query($conection," UPDATE `Electrico` SET `Foto` = '$nombreArchivo', `Nombre` = '$txtNombre', `Descripcion` = '$txtDescripcion', `Precio` = '$txtPrecio' WHERE `Electrico`.`ID_Elect` = '$txtID'");
 echo '<script language="javascript">alert("Electrico Modificado Correctamente");</script>';
header('location: MantElectrico.php');

    break;
     case "btnEliminar":

$query =mysqli_query($conection,"DELETE FROM `Electrico` WHERE `Electrico`.`ID_Elect` = $txtID");
   echo '<script language="javascript">alert("Agregada Nueva Electrico");</script>';
  header('location: MantElectrico.php');
...
<div class="container">
  <br>
  <h1>Mantenimiento Piezas Electricas</h1>
  <br>
  <form action="" method="post" enctype="multipart/form-data">

   <label for="name">ID Electrico</label>
...
<?php
 ...
 $query=mysqli_query($conection,"SELECT*FROM Electrico");
$lista=mysqli_fetch_all($query,MYSQLI_ASSOC);

 foreach ($lista as $items) { ?>
  <tr>
    <td><?php echo $items['ID_Elect'] ?></td>
...
<input type="hidden" name="IDAD"  value="<?php echo $items['ID_Elect']; ?>">
