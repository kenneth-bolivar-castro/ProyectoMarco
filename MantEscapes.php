...
$query =mysqli_query($conection,"INSERT INTO `Escapes` (`Foto`, `Nombre`, `Descripcion`, `Precio`)
   ...
   header('location: MantEscapes.php');
    break;
     case "btnModificar":
 ...
 $query =mysqli_query($conection," UPDATE `Escapes` SET `Foto` = '$nombreArchivo', `Nombre` = '$txtNombre', `Descripcion` = '$txtDescripcion', `Precio` = '$txtPrecio' WHERE `Escapes`.`ID_Escape` = '$txtID'");
 echo '<script language="javascript">alert("Escapes Modificado Correctamente");</script>';
header('location: MantEscapes.php');

    break;
     case "btnEliminar":

$query =mysqli_query($conection,"DELETE FROM `Escapes` WHERE `Escapes`.`ID_Escape` = $txtID");
   echo '<script language="javascript">alert("Agregada Nueva Escapes");</script>';
  header('location: MantEscapes.php');
...
<head>
  <meta charset="utf-8">
  <title>Mantenimiento Escapes</title>
...
<div class="container">
  <br>
  <h1>Mantenimiento para Escapes</h1>
  <br>
  <form action="" method="post" enctype="multipart/form-data">

   <label for="name">ID Escapes</label>
...
<?php
 ...
 $query=mysqli_query($conection,"SELECT*FROM Escapes");
$lista=mysqli_fetch_all($query,MYSQLI_ASSOC);

 foreach ($lista as $items) { ?>
  <tr>
    <td><?php echo $items['ID_Escape'] ?></td>
